...
ilu_qcopy_to_ucol(
	      ...
	      int	 *repfnz,  /* in */
	      int	 *perm_r,  /* in */
	      quadreal	 *dense,   /* modified - reset to zero on return */
	      int  	 drop_rule,/* in */
	      milu_t	 milu,	   /* in */
	      quadreal	 drop_tol, /* in */
	      int	 quota,    /* maximum nonzero entries allowed */
	      quadreal	 *sum,	   /* out - the sum of dropped entries */
	      int	 *nnzUj,   /* in - out */
	      GlobalLU_t *Glu,	   /* modified */
	      quadreal	 *work	   /* working space with minimum size n,
 ...
 */
    ...
    quadreal d_max = 0.0, d_min = 1.0 / qmach("Safe minimum");
    quadreal tmp;
    quadreal zero = 0.0;
    int i_1 = 1;
    ...
    *sum = zero;
	...
	if ( ksupno != jsupno ) { /* Should go into ucol[] */
		...
		while ( new_next > nzumax ) {
		    if ((mem_error = qLUMemXpand(jcol, nextu, UCOL, &nzumax,
			    Glu)) != 0)
			return (mem_error);
		    ucol = Glu->ucol;
		    if ((mem_error = qLUMemXpand(jcol, nextu, USUB, &nzumax,
			    Glu)) != 0)
			return (mem_error);
		    usub = Glu->usub;
		    lsub = Glu->lsub;
	...
	if (quota > 0) {
		...
		qcopy_(&m, &ucol[xusub[jcol]], &i_1, work, &i_1);
		tol = qqselect(m, work, quota);
#if 0
		A = &ucol[xusub[jcol]];
