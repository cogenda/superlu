 ...
 * =======
 *
 * QGSTRF computes an LU factorization of a general sparse m-by-n
...
void
qgstrf (superlu_options_t *options, SuperMatrix *A,
        ...
        SuperLUStat_t *stat, int *info)
{
    ...
    int       nzlumax;
    quadreal fill_ratio = sp_ienv(6);  /* estimated fill ratio */
    ...
    *info = qLUMemInit(fact, work, lwork, m, n, Astore->nnz,
	...
	if ( relax_end[jcol] != EMPTY ) { /* start of a relaxed snode */
	    ...
	    if ( (*info = qsnode_dfs(jcol, kcol, asub, xa_begin, xa_end,
				    xprune, marker, Glu)) != 0 )
		return;
		...
		if ( (*info = qpivotL(icol, diag_pivot_thresh, &usepr, perm_r,
				      iperm_r, iperm_c, &pivrow, Glu, stat)) )
		    if ( iinfo == 0 ) iinfo = *info;
		
#ifdef DEBUG
		qprint_lu_col("[1]: ", icol, pivrow, xprune, Glu);
#endif
	    ...
	    qpanel_dfs(m, panel_size, jcol, A, perm_r, &nseg1,
		...
		nseg = nseg1;	/* Begin after all the panel segments */
	    	...
	    	if ((*info = qcolumn_dfs(m, jj, perm_r, &nseg, &panel_lsub[k],
		...
		if ((*info = qcopy_to_ucol(jj, nseg, segrep, &repfnz[k],
					  perm_r, &dense[k], Glu)) != 0)
		    return;

	    	if ( (*info = qpivotL(jj, diag_pivot_thresh, &usepr, perm_r,
				      iperm_r, iperm_c, &pivrow, Glu, stat)) )
		    if ( iinfo == 0 ) iinfo = *info;

		/* Prune columns (0:jj-1) using column jj */
	    	qpruneL(jj, perm_r, pivrow, nseg, segrep,
                        &repfnz[k], xprune, Glu);
...
#ifdef DEBUG
		qprint_lu_col("[2]: ", jj, pivrow, xprune, Glu);
#endif
