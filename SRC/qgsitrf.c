 ...
 * =======
 *
 * QGSITRF computes an ILU factorization of a general sparse m-by-n
 ...
 *	    Stype = SLU_NCP; Dtype = SLU_Q; Mtype = SLU_GE.
 *
 * relax    (input) int
 ...
 *	    Stype = SLU_SC, Dtype = SLU_Q, Mtype = SLU_TRLU.
 *
 * U	    (output) SuperMatrix*
 *	    The factor U from the factorization Pr*A*Pc=L*U. Use column-wise
 *	    storage scheme, i.e., U has types: Stype = SLU_NC,
 *	    Dtype = SLU_Q, Mtype = SLU_TRU.
 *
...
void
qgsitrf(superlu_options_t *options, SuperMatrix *A, int relax, int panel_size,
	...
	SuperLUStat_t *stat, int *info)
{
    ...
    quadreal    *amax; 
    quadreal    drop_sum;
    quadreal alpha, omega;  /* used in MILU, mimicing DRIC */
    quadreal    *swork2;	   /* used by the second dropping rule */
    ...
    *info = qLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size,
    ...
    qSetRWork(m, panel_size, swork, &dense, &tempv);
    ...
    amax = (quadreal *) SUPERLU_MALLOC(panel_size * sizeof(quadreal));
    if (drop_rule & DROP_SECONDARY)
	swork2 = SUPERLU_MALLOC(n * sizeof(quadreal));
    else
	swork2 = NULL;
	...
	if ( relax_end[jcol] != EMPTY ) { /* start of a relaxed snode */
		...
		/* Drop small rows */
		i = ilu_qdrop_row(options, first, last, tol_L, quota, &nnzLj,
	    ...
	    if ( (*info = ilu_qsnode_dfs(jcol, kcol, asub, xa_begin, xa_end,
					 marker, Glu)) != 0 )
		return;
		...
		if ((*info = qLUMemXpand(jcol, nextlu, LUSUP, &nzlumax, Glu)))
		    return;
	    }
		...
		qsnode_bmod(icol, jsupno, fsupc, dense, tempv, Glu, stat);

		if (usepr) pivrow = iperm_r[icol];
		fill_tol = pow(fill_ini, 1.0 - (double)icol / (double)min_mn);
		if ( (*info = ilu_qpivotL(icol, diag_pivot_thresh, &usepr,
	    ...
	    ilu_qpanel_dfs(m, panel_size, jcol, A, perm_r, &nseg1,
                          dense, amax, panel_lsub, segrep, repfnz,
                          marker, parent, xplore, Glu);

	    /* numeric sup-panel updates in topological order */
	    qpanel_bmod(m, panel_size, jcol, nseg1, dense,
		...
		if ((*info = ilu_qcolumn_dfs(m, jj, perm_r, &nseg,
					     &panel_lsub[k], segrep, &repfnz[k],
					     marker, parent, xplore, Glu)))
		    return;

		/* Numeric updates */
		if ((*info = qcolumn_bmod(jj, (nseg - nseg1), &dense[k],
		...
		if (xlsub[jj + 1] == xlsub[jj]) {
		    ...
		    nextl = xlsub[jj] + 1;
		    if (nextl >= nzlmax) {
			int error = qLUMemXpand(jj, nextl, LSUB, &nzlmax, Glu);
		    ...
		    xlusup[jj + 1]++;
		    ((quadreal *) Glu->lusup)[xlusup[jj]] = zero;
		...
		if ((*info = ilu_qcopy_to_ucol(jj, nseg, segrep, &repfnz[k],
		...
		if ( (*info = ilu_qpivotL(jj, diag_pivot_thresh, &usepr, perm_r,
		...
		if (jj > 0 && supno[jj] > supno[jj - 1] && jj < last_drop) {
		    ...
		    /* Drop small rows */
		    i = ilu_qdrop_row(options, first, last, tol_L, quota,
    ...
    *info = iinfo;
    ...
    qLUWorkFree(iwork, swork, Glu); /* Free work space and compress storage */
	...
	((SCformat *)L->Store)->nzval = (quadreal *) Glu->lusup;
	((SCformat *)L->Store)->nzval_colptr = Glu->xlusup;
	((SCformat *)L->Store)->rowind = Glu->lsub;
	((SCformat *)L->Store)->rowind_colptr = Glu->xlsub;
	((NCformat *)U->Store)->nnz = nnzU;
	((NCformat *)U->Store)->nzval = (quadreal *) Glu->ucol;
	((NCformat *)U->Store)->rowind = Glu->usub;
	((NCformat *)U->Store)->colptr = Glu->xusub;
    } else {
	qCreate_SuperNode_Matrix(L, A->nrow, min_mn, nnzL,
              (quadreal *) Glu->lusup, Glu->xlusup,
              Glu->lsub, Glu->xlsub, Glu->supno, Glu->xsup,
	      SLU_SC, SLU_Q, SLU_TRLU);
	qCreate_CompCol_Matrix(U, min_mn, min_mn, nnzU,
	      (quadreal *) Glu->ucol, Glu->usub, Glu->xusub,
	      SLU_NC, SLU_Q, SLU_TRU);
    }
