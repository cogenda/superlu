 ...
 * =======
 *
 * QGSISX computes an approximate solutions of linear equations
 ...
 *	   Stype = SLU_NC or SLU_NR, Dtype = SLU_Q, Mtype = SLU_GE.
...
void
qgsisx(superlu_options_t *options, SuperMatrix *A, int *perm_c, int *perm_r,
       int *etree, char *equed, quadreal *R, quadreal *C,
       SuperMatrix *L, SuperMatrix *U, void *work, int lwork,
       SuperMatrix *B, SuperMatrix *X,
       quadreal *recip_pivot_growth, quadreal *rcond,
       GlobalLU_t *Glu, mem_usage_t *mem_usage, SuperLUStat_t *stat, int *info)
{
	...
	*info = -6;
	...
	quadreal *nzval = (quadreal *)Astore->nzval;
	...
	if ( mc64 ) {
	    ...
	    info1 = qldperm(5, n, nnz, colptr, rowind, nzval, perm, R, C);
	...
	t0 = SuperLU_timer_();
	qgsitrf(options, &AC, relax, panel_size, etree, work, lwork,
	...
	*recip_pivot_growth = qPivotGrowth(A->ncol, AA, perm_c, L, U);
	...
	if ( *rcond < qmach("E") && *info == 0) *info = A->ncol + 1;
    }

    if ( nofact ) {
	ilu_qQuerySpace(L, U, mem_usage);
