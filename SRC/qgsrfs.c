 ...
 * October 15, 2003
 *
 * Modified from lapack routine QGERFS
 * Last modified: December 3, 2015
 * </pre>
 */
/*
 * File name:	qgsrfs.c
 * History:     Modified from lapack routine QGERFS
 */
#include <math.h>
#include "slu_qdefs.h"
 ...
 *   =======   
 *
 *   QGSRFS improves the computed solution to a system of linear   
 ...
 *           Stype = SLU_NC, Dtype = SLU_Q, Mtype = SLU_GE.
 *    
 *   L       (input) SuperMatrix*
 *	     The factor L from the factorization Pr*A*Pc=L*U. Use
 *           compressed row subscripts storage for supernodes, 
 *           i.e., L has types: Stype = SLU_SC, Dtype = SLU_Q, Mtype = SLU_TRLU.
 * 
 *   U       (input) SuperMatrix*
 *           The factor U from the factorization Pr*A*Pc=L*U as computed by
 *           sgstrf(). Use column-wise storage scheme, 
 *           i.e., U has types: Stype = SLU_NC, Dtype = SLU_Q, Mtype = SLU_TRU.
 *
 ...
 *                  by diag(R)*A*diag(C).
 *
 *   R       (input) quadreal*, dimension (A->nrow)
 *           The row scale factors for A.
 *           If equed = 'R' or 'B', A is premultiplied by diag(R).
 *           If equed = 'N' or 'C', R is not accessed.
 * 
 *   C       (input) quadreal*, dimension (A->ncol)
 *           The column scale factors for A.
 *           If equed = 'C' or 'B', A is postmultiplied by diag(C).
 *           If equed = 'N' or 'R', C is not accessed.
 *
 *   B       (input) SuperMatrix*
 *           B has types: Stype = SLU_DN, Dtype = SLU_Q, Mtype = SLU_GE.
 *           The right hand side matrix B.
 *           if equed = 'R' or 'B', B is premultiplied by diag(R).
 *
 *   X       (input/output) SuperMatrix*
 *           X has types: Stype = SLU_DN, Dtype = SLU_Q, Mtype = SLU_GE.
 *           On entry, the solution matrix X, as computed by sgstrs().
 *           On exit, the improved solution matrix X.
 *           if *equed = 'C' or 'B', X should be premultiplied by diag(C)
 *               in order to obtain the solution to the original system.
 *
 *   FERR    (output) quadreal*, dimension (B->ncol)   
 ...
 *
 *   BERR    (output) quadreal*, dimension (B->ncol)   
 ...
 */
void
qgsrfs(trans_t trans, SuperMatrix *A, SuperMatrix *L, SuperMatrix *U,
       int *perm_c, int *perm_r, char *equed, quadreal *R, quadreal *C,
       SuperMatrix *B, SuperMatrix *X, quadreal *ferr, quadreal *berr,
...
#define ITMAX 5
    ...
    extern int qlacon2_(int *, quadreal *, quadreal *, int *, quadreal *, int *, int []);
#ifdef _CRAY
    extern int QCOPY(int *, quadreal *, int *, quadreal *, int *);
    extern int QSAXPY(int *, quadreal *, quadreal *, int *, quadreal *, int *);
#else
    extern int qcopy_(int *, quadreal *, int *, quadreal *, int *);
    extern int qaxpy_(int *, quadreal *, quadreal *, int *, quadreal *, int *);
#endif
    ...
    *info = 0;
    ...
    if ( !notran && trans != TRANS && trans != CONJ ) *info = -1;
	      ...
	      A->Stype != SLU_NC || A->Dtype != SLU_Q || A->Mtype != SLU_GE )
	*info = -2;
    else if ( L->nrow != L->ncol || L->nrow < 0 ||
 	      L->Stype != SLU_SC || L->Dtype != SLU_Q || L->Mtype != SLU_TRLU )
	*info = -3;
    else if ( U->nrow != U->ncol || U->nrow < 0 ||
 	      U->Stype != SLU_NC || U->Dtype != SLU_Q || U->Mtype != SLU_TRU )
	*info = -4;
    else if ( ldb < SUPERLU_MAX(0, A->nrow) ||
 	      B->Stype != SLU_DN || B->Dtype != SLU_Q || B->Mtype != SLU_GE )
        *info = -10;
    else if ( ldx < SUPERLU_MAX(0, A->nrow) ||
 	      X->Stype != SLU_DN || X->Dtype != SLU_Q || X->Mtype != SLU_GE )
	*info = -11;
    if (*info != 0) {
	i = -(*info);
    ...
    work = quadrealMalloc(2*A->nrow);
    rwork = (quadreal *) SUPERLU_MALLOC( A->nrow * sizeof(quadreal) );
	...
	*(unsigned char *)transc = 'C';
	...
	while (1) { /* Loop until stopping criterion is satisfied. */
...
#ifdef _CRAY
	    QCOPY(&A->nrow, Bptr, &ione, work, &ione);
#else
	    qcopy_(&A->nrow, Bptr, &ione, work, &ione);
#endif
	    sp_qgemv(transc, ndone, A, Xptr, ione, done, work, ione);
		...
		qgstrs (trans, L, U, perm_c, perm_r, &Bjcol, stat, info);
		
#ifdef _CRAY
		QAXPY(&A->nrow, &done, work, &ione,
		       &Xmat[j*ldx], &ione);
#else
		qaxpy_(&A->nrow, &done, work, &ione,
		       &Xmat[j*ldx], &ione);
#endif
		lstres = berr[j];
		++count;
	...
	do {
	    qlacon2_(&A->nrow, &work[A->nrow], work,
		...
		qgstrs (transt, L, U, perm_c, perm_r, &Bjcol, stat, info);
