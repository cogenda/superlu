...
void
qCreate_CompCol_Matrix(SuperMatrix *A, int m, int n, int nnz, 
		       quadreal *nzval, int *rowind, int *colptr,
...
void
qCreate_CompRow_Matrix(SuperMatrix *A, int m, int n, int nnz, 
		       quadreal *nzval, int *colind, int *rowptr,
		       Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    NRformat *Astore;
...
void
qCopy_CompCol_Matrix(SuperMatrix *A, SuperMatrix *B)
{
    ...
    for (i = 0; i < nnz; ++i)
	((quadreal *)Bstore->nzval)[i] = ((quadreal *)Astore->nzval)[i];
...
void
qCreate_Dense_Matrix(SuperMatrix *X, int m, int n, quadreal *x, int ldx,
		    Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    DNformat    *Xstore;
    ...
    Xstore->lda = ldx;
    Xstore->nzval = (quadreal *) x;
}

void
qCopy_Dense_Matrix(int M, int N, quadreal *X, int ldx,
			quadreal *Y, int ldy)
{
...
void
qCreate_SuperNode_Matrix(SuperMatrix *L, int m, int n, int nnz, 
			quadreal *nzval, int *nzval_colptr, int *rowind,
 ...
 */
void
qCompRow_to_CompCol(int m, int n, int nnz, 
		    quadreal *a, int *colind, int *rowptr,
		    quadreal **at, int **rowind, int **colptr)
{
    register int i, j, col, relpos;
    int *marker;

    /* Allocate storage for another copy of the matrix. */
    *at = (quadreal *) quadrealMalloc(nnz);
...
void
qPrint_CompCol_Matrix(char *what, SuperMatrix *A)
{
    NCformat     *Astore;
    register int i,n;
    quadreal       *dp;
    ...
    n = A->ncol;
    Astore = (NCformat *) A->Store;
    dp = (quadreal *) Astore->nzval;
    printf("nrow %d, ncol %d, nnz %d\n", A->nrow,A->ncol,Astore->nnz);
    printf("nzval: ");
    for (i = 0; i < Astore->colptr[n]; ++i) printf("%e  ", (double)dp[i]);
    printf("\nrowind: ");
...
void
qPrint_SuperNode_Matrix(char *what, SuperMatrix *A)
{
    SCformat     *Astore;
...
void
qPrint_Dense_Matrix(char *what, SuperMatrix *A)
{
    ...
    printf("\nnzval: ");
    for (j = 0; j < A->ncol; ++j) {
        for (i = 0; i < A->nrow; ++i) printf("%e  ", (double)dp[i + j*lda]);
        printf("\n");
    }
    printf("\n");
    fflush(stdout);
 ...
 */
void
qprint_lu_col(char *msg, int jcol, int pivrow, int *xprune, GlobalLU_t *Glu)
{
    int     i, k, fsupc;
    int     *xsup, *supno;
    int     *xlsub, *lsub;
 ...
 */
void qcheck_tempv(int n, quadreal *tempv)
{
    int i;
...
void
qGenXtrue(int n, int nrhs, quadreal *x, int ldx)
{
    int  i, j;
    for (j = 0; j < nrhs; ++j)
	for (i = 0; i < n; ++i) {
	    x[i + j*ldx] = 1.0;/* + (quadreal)(i+1.)/n;*/
	}
}
 ...
 */
void
qFillRHS(trans_t trans, int nrhs, quadreal *x, int ldx,
         SuperMatrix *A, SuperMatrix *B)
{
    ...
    sp_qgemm(transc, "N", A->nrow, nrhs, A->ncol, one, A,
	     x, ldx, zero, rhs, ldc);

}

/*! \brief Fills a quadreal precision array with a given value.
 */
void 
qfill(quadreal *a, int alen, quadreal dval)
{
    register int i;
 ...
 */
void qinf_norm_error(int nrhs, SuperMatrix *X, quadreal *xtrue)
{
    DNformat *Xstore;
    quadreal err, xnorm;
    quadreal *Xmat, *soln_work;
    int i, j;
...
void
qPrintPerf(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage,
           quadreal rpg, quadreal rcond, quadreal *ferr,
           quadreal *berr, char *equed, SuperLUStat_t *stat)
{
    SCformat *Lstore;
    NCformat *Ustore;
    double   *utime;
    ...
    printf("NUM:\t%e\t%e\t%e\t%e\t%s\n",
	   (double)rpg, (double)rcond, (double)ferr[0], (double)berr[0], equed);
...
int
print_quadreal_vec(char *what, int n, quadreal *vec)
{
    int i;
    printf("%s: n %d\n", what, n);
