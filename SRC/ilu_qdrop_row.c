...
extern void qswap_(int *, quadreal [], int *, quadreal [], int *);
extern void qaxpy_(int *, quadreal *, quadreal [], int *, quadreal [], int *);
extern void qcopy_(int *, quadreal [], int *, quadreal [], int *);
extern quadreal qasum_(int *, quadreal *, int *);
extern quadreal qnrm2_(int *, quadreal *, int *);
extern int iqamax_(int *, quadreal [], int *);

static quadreal *A;  /* used in _compare_ only */
...
int ilu_qdrop_row(
	...
	quadreal drop_tol,    /* dropping parameter */
	int    quota,	    /* maximum nonzero entries allowed */
	int    *nnzLj,	    /* in/out number of nonzeros in L(:, 1:last) */
	quadreal *fill_tol,   /* in/out - on exit, fill_tol=-num_zero_pivots,
			     * does not change if options->ILU_MILU != SMILU1 */
	GlobalLU_t *Glu,    /* modified */
	quadreal swork[],   /* working space
	                     * the length of swork[] should be no less than
			     * the number of rows in the supernode */
	quadreal swork2[], /* working space with the same size as swork[],
			     ...
			     * the working supernode. */ )
{
    ...
    quadreal alpha = pow((double)(Glu->n), -1.0 / options->ILU_MILU_Dim);
	...
	*nnzLj += m * n;
	...
	{
	    case ONE_NORM:
		temp[i] = qasum_(&n, &lusup[xlusup_first + i], &m) / (double)n;
		break;
	    case TWO_NORM:
		temp[i] = qnrm2_(&n, &lusup[xlusup_first + i], &m)
		    / sqrt((double)n);
		break;
	    case INF_NORM:
	    default:
		k = iqamax_(&n, &lusup[xlusup_first + i], &m) - 1;
	...
	if (drop_rule & DROP_BASIC && temp[i] < drop_tol)
	{
		...
		switch (milu)
		{
		    case SMILU_1:
		    case SMILU_2:
			qaxpy_(&n, &one, &lusup[xlusup_first + i], &m,
		...
		qcopy_(&n, &lusup[xlusup_first + m1], &m,
                       &lusup[xlusup_first + i], &m);
	    } /* if (r > 1) */
	    else /* move to last row */
	    {
		qswap_(&n, &lusup[xlusup_first + m1], &m,
			&lusup[xlusup_first + i], &m);
		if (milu == SMILU_3)
		    for (j = 0; j < n; j++) {
	...
	if (quota > n)
	{
	    ...
	    {
		int len = m1 - n + 1;
		qcopy_(&len, swork, &i_1, swork2, &i_1);
		tol = qqselect(len, swork2, quota - n);
#if 0
