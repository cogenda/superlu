...
/* Internal prototypes */
void  *qexpand (int *, MemType,int, int, GlobalLU_t *);
int   qLUWorkInit (int, int, int, int **, quadreal **, GlobalLU_t *);
void  copy_mem_quadreal (int, void *, void *);
void  qStackCompress (GlobalLU_t *);
void  qSetupSpace (void *, int, GlobalLU_t *);
void  *quser_malloc (int, int, GlobalLU_t *);
void  quser_free (int, int, GlobalLU_t *);
 ...
 */
void qSetupSpace(void *work, int lwork, GlobalLU_t *Glu)
{
    if ( lwork == 0 ) {
	...
	Glu->stack.used = 0;
	Glu->stack.top1 = 0;
	Glu->stack.top2 = (lwork/16)*16; /* must be word addressable */
...
void *quser_malloc(int bytes, int which_end, GlobalLU_t *Glu)
{
    void *buf;
...
void quser_free(int bytes, int which_end, GlobalLU_t *Glu)
{
    if ( which_end == HEAD ) {
	Glu->stack.top1 -= bytes;
    } else {
 ...
 * </pre>
 */
int qQuerySpace(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage)
{
    SCformat *Lstore;
    NCformat *Ustore;
 ...
 * </pre>
 */
int ilu_qQuerySpace(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage)
{
    SCformat *Lstore;
    NCformat *Ustore;
 ...
 */
int
qLUMemInit(fact_t fact, void *work, int lwork, int m, int n, int annz,
	  int panel_size, quadreal fill_ratio, SuperMatrix *L, SuperMatrix *U,
          GlobalLU_t *Glu, int **iwork, quadreal **dwork)
{
	...
	if ( Glu->MemModel == SYSTEM ) {
	    ...
	    xsup   = (int *)quser_malloc((n+1) * iword, HEAD, Glu);
	    supno  = (int *)quser_malloc((n+1) * iword, HEAD, Glu);
	    xlsub  = (int *)quser_malloc((n+1) * iword, HEAD, Glu);
	    xlusup = (int *)quser_malloc((n+1) * iword, HEAD, Glu);
	    xusub  = (int *)quser_malloc((n+1) * iword, HEAD, Glu);
	}

	lusup = (quadreal *) qexpand( &nzlumax, LUSUP, 0, 0, Glu );
	ucol  = (quadreal *) qexpand( &nzumax, UCOL, 0, 0, Glu );
	lsub  = (int *)   qexpand( &nzlmax, LSUB, 0, 0, Glu );
	usub  = (int *)   qexpand( &nzumax, USUB, 0, 1, Glu );
	...
	while ( !lusup || !ucol || !lsub || !usub ) {
		...
		quser_free((nzlumax+nzumax)*dword+(nzlmax+nzumax)*iword,
                            HEAD, Glu);
	    }
	    nzlumax /= 2;
	    nzumax /= 2;
		...
		return (qmemory_usage(nzlmax, nzumax, nzlumax, n) + n);
	    }
#if ( PRNTlevel >= 1)
	    printf("qLUMemInit() reduce size: nzlmax %ld, nzumax %ld\n", 
		   nzlmax, nzumax);
	    fflush(stdout);
#endif
	    lusup = (quadreal *) qexpand( &nzlumax, LUSUP, 0, 0, Glu );
    ...
    info = qLUWorkInit(m, n, panel_size, iwork, dwork, Glu);
    if ( info )
	return ( info + qmemory_usage(nzlmax, nzumax, nzlumax, n) + n);
...
int
qLUWorkInit(int m, int n, int panel_size, int **iworkptr, 
            quadreal **dworkptr, GlobalLU_t *Glu)
{
    ...
    dsize = (m * panel_size +
	     NUM_TEMPV(m,panel_size,maxsuper,rowblk)) * sizeof(quadreal);
    
    if ( Glu->MemModel == SYSTEM ) 
	*iworkptr = (int *) intCalloc(isize/sizeof(int));
    else
	*iworkptr = (int *) quser_malloc(isize, TAIL, Glu);
    if ( ! *iworkptr ) {
	fprintf(stderr, "qLUWorkInit: malloc fails for local iworkptr[]\n");
	return (isize + n);
    }

    if ( Glu->MemModel == SYSTEM )
	*dworkptr = (quadreal *) SUPERLU_MALLOC(dsize);
    else {
	*dworkptr = (quadreal *) quser_malloc(dsize, TAIL, Glu);
	if ( NotDoubleAlign(*dworkptr) ) {
	    old_ptr = *dworkptr;
	    *dworkptr = (quadreal*) DoubleAlign(*dworkptr);
	    *dworkptr = (quadreal*) ((double*)*dworkptr - 1);
 ...
 */
void
qSetRWork(int m, int panel_size, quadreal *dworkptr,
	 quadreal **dense, quadreal **tempv)
{
    quadreal zero = 0.0;
    ...
    *tempv = *dense + panel_size*m;
    qfill (*dense, m * panel_size, zero);
    qfill (*tempv, NUM_TEMPV(m,panel_size,maxsuper,rowblk), zero);     
}
	
/*! \brief Free the working storage used by factor routines.
 */
void qLUWorkFree(int *iwork, quadreal *dwork, GlobalLU_t *Glu)
{
    if ( Glu->MemModel == SYSTEM ) {
	SUPERLU_FREE (iwork);
	SUPERLU_FREE (dwork);
	   ...
	   )
{
...
#ifdef DEBUG    
    printf("qLUMemXpand(): jcol %d, next %d, maxlen %d, MemType %d\n",
	   jcol, next, *maxlen, mem_type);
#endif    

    if (mem_type == USUB) 
    	new_mem = qexpand(maxlen, mem_type, next, 1, Glu);
    else
	new_mem = qexpand(maxlen, mem_type, next, 0, Glu);
	...
	int    nzlumax = Glu->nzlumax;
    	...
    	return (qmemory_usage(nzlmax, nzumax, nzlumax, Glu->n) + Glu->n);
    }
...
void
copy_mem_quadreal(int howmany, void *old, void *new)
{
    register int i;
    quadreal *dold = old;
    quadreal *dnew = new;
	...
	)
{
	...
	if ( Glu->num_expansions != 0 ) {
		...
		copy_mem_quadreal(len_to_copy, expanders[type].mem, new_mem);
	    }
	...
	if ( Glu->num_expansions == 0 ) {
	    new_mem = quser_malloc(new_len * lword, HEAD, Glu);
...
qStackCompress(GlobalLU_t *Glu)
{
    ...
    dfrom = ucol;
    dto = (quadreal *)((char*)lusup + xlusup[ndim] * dword);
    copy_mem_quadreal(xusub[ndim], dfrom, dto);
    ucol = dto;
...
#ifdef DEBUG
    printf("qStackCompress: fragment %d\n", fragment);
 ...
 */
void
qallocateA(int n, int nnz, quadreal **a, int **asub, int **xa)
{
    *a    = (quadreal *) quadrealMalloc(nnz);
    *asub = (int *) intMalloc(nnz);
    *xa   = (int *) intMalloc(n+1);
}


quadreal *quadrealMalloc(int n)
{
    quadreal *buf;
    buf = (quadreal *) SUPERLU_MALLOC((size_t)n * sizeof(quadreal)); 
    if ( !buf ) {
	ABORT("SUPERLU_MALLOC failed for buf in quadrealMalloc()\n");
    }
    return (buf);
}

quadreal *quadrealCalloc(int n)
{
    quadreal *buf;
    register int i;
    quadreal zero = 0.0;
    buf = (quadreal *) SUPERLU_MALLOC((size_t)n * sizeof(quadreal));
    if ( !buf ) {
	ABORT("SUPERLU_MALLOC failed for buf in quadrealCalloc()\n");
    }
...
int qmemory_usage(const int nzlmax, const int nzumax, 
