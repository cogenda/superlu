 ...
 * =======
 *
 * QGSSVX solves the system of linear equations A*X=B or A'*X=B, using
 ...
 *
 * R       (input/output) quadreal*, dimension (A->nrow)
 ...
 * 
 * C       (input/output) quadreal*, dimension (A->ncol)
 ...
 *         L has types: Stype = SLU_SC, Dtype = SLU_Q, Mtype = SLU_TRLU.
 *
 ...
 *         Stype = SLU_NC, Dtype = SLU_Q, Mtype = SLU_TRU.
 *
 ...
 *         and equed = 'R' or 'B'.
 *
 * recip_pivot_growth (output) quadreal*
 *         The reciprocal pivot growth factor max_j( norm(A_j)/norm(U_j) ).
 *         The infinity norm is used. If recip_pivot_growth is much less
 *         than 1, the stability of the LU factorization could be poor.
 *
 * rcond   (output) quadreal*
...
void
qgssvx(superlu_options_t *options, SuperMatrix *A, int *perm_c, int *perm_r,
       int *etree, char *equed, quadreal *R, quadreal *C,
       SuperMatrix *L, SuperMatrix *U, void *work, int lwork,
       SuperMatrix *B, SuperMatrix *X, quadreal *recip_pivot_growth, 
       quadreal *rcond, quadreal *ferr, quadreal *berr, 
       GlobalLU_t *Glu, mem_usage_t *mem_usage, SuperLUStat_t *stat, int *info )
{
    ...
    char      norm[1];
    int       i, j, info1;
    quadreal    amax, anorm, bignum, smlnum, colcnd, rowcnd, rcmax, rcmin;
    int       relax, panel_size;
    quadreal    diag_pivot_thresh;
    double    t0;      /* temporary time */
    double    *utime;

    /* External functions */
    extern quadreal qlangs(char *, SuperMatrix *);
	...
	*(unsigned char *)equed = 'N';
	...
	smlnum = qmach("Safe minimum");   /* lamch_("Safe minimum"); */
	bignum = 1. / smlnum;
    }

#if 0
printf("qgssvx: Fact=%4d, Trans=%4d, equed=%c\n",
	...
	*info = -6;
	...
	qCreate_CompCol_Matrix(AA, A->ncol, A->nrow, Astore->nnz, 
	...
	qgsequ(AA, R, C, &rowcnd, &colcnd, &amax, &info1);
	
	if ( info1 == 0 ) {
	    /* Equilibrate matrix A. */
	    qlaqgs(AA, R, C, rowcnd, colcnd, amax, equed);
	    rowequ = strncmp(equed, "R", 1)==0 || strncmp(equed, "B", 1)==0;
	    colequ = strncmp(equed, "C", 1)==0 || strncmp(equed, "B", 1)==0;
	}
...
/*	printf("Factor PA = LU ... relax %d\tw %d\tmaxsuper %d\trowblk %d\n", 
	...
	t0 = SuperLU_timer_();
	qgstrf(options, &AC, relax, panel_size, etree,
    ...
    if ( *info > 0 ) {
	    ...
	    *recip_pivot_growth = qPivotGrowth(*info, AA, perm_c, L, U);
        }
	return;
    }
    ...
    if ( options->PivotGrowth ) {
        ...
        *recip_pivot_growth = qPivotGrowth(A->ncol, AA, perm_c, L, U);
    }
    ...
    if ( options->ConditionNumber ) {
	    ...
	    *(unsigned char *)norm = 'I';
        ...
        anorm = qlangs(norm, AA);
        qgscon(norm, L, U, anorm, rcond, stat, &info1);
    ...
    if ( nrhs > 0 ) {
        ...
        t0 = SuperLU_timer_();
        qgstrs (trant, L, U, perm_c, perm_r, X, stat, &info1);
        ...
        if ( options->IterRefine != NOREFINE ) {
            qgsrfs(trant, AA, L, U, perm_c, perm_r, equed, R, C, B,
    ...
    if ( options->ConditionNumber ) {
        ...
        if ( *rcond < qmach("E") ) *info = A->ncol + 1;
    }

    if ( nofact ) {
        qQuerySpace(L, U, mem_usage);
