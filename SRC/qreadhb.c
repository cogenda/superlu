...
int qParseIntFormat(char *buf, int *num, int *size)
{
    char *tmp;
...
int qParseFloatFormat(char *buf, int *num, int *size)
{
    char *tmp, *period;
...
int qReadValues(FILE *fp, int n, quadreal *destination, int perline, int persize)
{
...
FormFullA(int n, int *nonz, quadreal **nzval, int **rowind, int **colptr)
{
    ...
    if ( !(t_val = (quadreal*) SUPERLU_MALLOC( *nonz * sizeof(quadreal)) ) )
    ...
    if ( !(a_val = (quadreal*) SUPERLU_MALLOC( new_nnz * sizeof(quadreal)) ) )
...
void
qreadhb(FILE *fp, int *nrow, int *ncol, int *nonz,
	quadreal **nzval, int **rowind, int **colptr)
{
    ...
    qallocateA(*ncol, *nonz, nzval, rowind, colptr);

    /* Line 4: format statement */
    fscanf(fp, "%16c", buf);
    qParseIntFormat(buf, &colnum, &colsize);
    fscanf(fp, "%16c", buf);
    qParseIntFormat(buf, &rownum, &rowsize);
    fscanf(fp, "%20c", buf);
    qParseFloatFormat(buf, &valnum, &valsize);
    fscanf(fp, "%20c", buf);
    qDumpLine(fp);

    /* Line 5: right-hand side */    
    if ( rhscrd ) qDumpLine(fp); /* skip RHSFMT */
    ...
    if ( numer_lines ) {
        qReadValues(fp, *nonz, *nzval, valnum, valsize);
    }
