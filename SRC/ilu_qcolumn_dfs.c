 ...
 * Purpose
 * =======
 *   ILU_QCOLUMN_DFS performs a symbolic factorization on column jcol, and
	   ...
	   )
{
	...
	if ( kperm == EMPTY ) {
	    ...
	    if ( nextl >= nzlmax ) {
		if ((mem_error = qLUMemXpand(jcol, nextl, LSUB, &nzlmax, Glu)))
		    return (mem_error);
		lsub = Glu->lsub;
	    }
		...
		do {
			...
			if ( chmark != jcol ) { /* Not reached yet */
				...
				lsub[nextl++] = kchild;
				if ( nextl >= nzlmax ) {
				    if ( (mem_error = qLUMemXpand(jcol,nextl,
					    LSUB,&nzlmax,Glu)) )
					return (mem_error);
				    lsub = Glu->lsub;
				}
